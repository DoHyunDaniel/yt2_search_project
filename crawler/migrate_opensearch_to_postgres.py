...
OS_CLIENT = OpenSearch(
    hosts=["http://yt2-opensearch:9200"], use_ssl=False, verify_certs=False
)
...
def get_opensearch_data():
    ...
    try:
        ...
        response = OS_CLIENT.search(
            index="videos",
            body={"query": {"match_all": {}}, "size": 1000},  # 모든 데이터 가져오기
        )
            ...
            with conn.cursor() as cur:
                for i, data in enumerate(data_list):
                    try:
                        ...
                        success_count += 1
                        logger.info(
                            f"[{i+1}/{len(data_list)}] 데이터 마이그레이션 완료: video_id={data['video_id']}"
                        )

                    except Exception as e:
                        error_count += 1
                        logger.error(
                            f"[{i+1}/{len(data_list)}] 데이터 마이그레이션 실패: video_id={data['video_id']}, 오류: {e}"
                        )
                        continue

                conn.commit()
                logger.info(
                    f"마이그레이션 완료: 성공 {success_count}개, 실패 {error_count}개"
                )
...
def save_channel(cur, data):
    ...
    try:
        # 채널이 이미 존재하는지 확인
        cur.execute(
            "SELECT id FROM yt2.channels WHERE channel_yid = %s", (data["channel_id"],)
        )
...
def save_video(cur, data, channel_id):
    ...
    try:
        # 비디오가 이미 존재하는지 확인
        cur.execute(
            "SELECT id FROM yt2.videos WHERE video_yid = %s", (data["video_id"],)
        )
