...
class YT2Crawler:
    ...
    def __init__(self):
        ...
        self.os_client = OpenSearch(
            ...
            http_auth=(
                os.getenv("OS_USER", "admin"),
                os.getenv("OS_PASSWORD", "App1234!@#"),
            ),
            use_ssl=False,
            verify_certs=False,
        )
    ...
    def search_videos(
        self, keyword: str, max_results: int = 50, days: int = 30
    ) -> List[Dict]:
        """키워드로 영상 검색"""
    ...
    def index_to_opensearch(self, video_data: Dict) -> bool:
        ...
        try:
            ...
            self.os_client.index(index="videos", id=video_data["video_id"], body=doc)
...
def main():
    ...
    parser.add_argument(
        "--max-results", type=int, default=50, help="키워드당 최대 결과 수"
    )
    parser.add_argument("--days", type=int, default=30, help="검색할 일수")
    parser.add_argument(
        "--dry-run", action="store_true", help="실제 크롤링 없이 테스트만 실행"
    )
